...
- 此插件的翻譯進度目前為 **部分翻譯** 。
## 抽卡紀錄功能與服務相關注意事項
1. 當使用者（下稱「您」）開啟抽卡紀錄功能（下稱「本功能」）時，視同已閱讀並同意本注意事項。
2. 您同意本插件基於提供非營利之紀錄、分析等目的，蒐集您於遊戲中的使用者ID、使用者名稱，以及抽卡紀錄（下稱「必要資料」）。
3. 必要資料中並不涉及[個人資料保護法第二條第一款](https://law.moj.gov.tw/LawClass/LawSingle.aspx?pcode=I0050021&flno=2)所定義之個人資料，故不適用該法。
4. 抽卡紀錄服務（下稱「本服務」）係指透過本功能上傳必要資料至本服務後，透過[本服務之網址](https://uma.gacha.chinosk6.cn/)查詢使用者抽卡相關紀錄與分析之服務。
5. 您可以拒絕提供完整之必要資料，但將可能無法正常使用該服務。
6. 使用本功能與本服務，不涉及向遊戲服務端上傳資料，也不涉及修改客戶端資料，故使用本功能與使用本插件之風險一致，不會增加額外風險。
7. 本功能與本服務之部分或全部之內容，均得不經預告隨時修改、暫停或終止。
8. 本注意事項得不經預告隨時修改。
